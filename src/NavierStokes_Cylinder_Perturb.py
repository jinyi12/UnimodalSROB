...
num_steps = int(T / dt)
ss_time_step_size = int((1/dt) / 200)

nominal_mu = 0.001
# test for +- 0.001, 0.005, 0.01
...
# mu_list = [nominal_mu*1.01, nominal_mu]
...
for j in range(len(mu_list)):
    ...
    mu = Constant(mesh, PETSc.ScalarType(mu_list[j]))  # Dynamic viscosity
    ...
    folder = Path(f"/data1/jy384/research/Data/UnimodalSROB/results/bp_mu_{names[j]}_rho_{rho_}/")
    ss_file = Path(f"/data1/jy384/research/Data/UnimodalSROB/snapshots/")
    velocity_folder = f"dfg2D-3-u_mu_{names[j]}_rho{rho_}.bp"
    pressure_folder= f"dfg2D-3-p_mu_{names[j]}_rho{rho_}.bp" 

    velocity_folder = folder / velocity_folder
    pressure_folder = folder / pressure_folder

    folder.mkdir(exist_ok=True, parents=True)
    vtx_u = VTXWriter(mesh.comm, velocity_folder, [u_], engine="BP4")
    vtx_p = VTXWriter(mesh.comm, pressure_folder, [p_], engine="BP4")
    vtx_u.write(t)
    vtx_p.write(t)
    progress = tqdm.autonotebook.tqdm(desc="Solving PDE", total=num_steps)
    for i in range(num_steps):
        time  = i * dt  # calculate the current time
        ...
        # if time within 4 and 5 seconds
        if (4 <= time < 5) & (i % ss_time_step_size == 0):
            print("Saving snapshots")
            # Collect snapshots
            snapshots_u.append(u_.vector.copy())
            snapshots_p.append(p_.vector.copy())
    ...
    np.savez_compressed(folder / f"snapshots_mu_{names[j]}_rho_{rho_}.npz", u=snapshots_u, p=snapshots_p) 
