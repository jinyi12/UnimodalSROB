...
def relative_error(S_exact, S_reconstructed, Sref):
    ...
    return np.linalg.norm(S_exact - S_reconstructed, "fro") / np.linalg.norm(
        S_exact - Sref, "fro"
    )
...
def representation_learning_obj(x, snapshot_index, Vr, Vbar, Xi, S, sref, p=2):
    ...
    return (
        ...
        - sref
        - (Vr @ x)
        - (Vbar @ Xi @ np.concatenate(polynomial_form(x, p), axis=0))
    )


def alternating_minimization(
    S, Sref, num_snapshots, max_iter, tol, gamma, r, q, p, initial_Shat=None
):
    """
    ...
    for niter in range(max_iter):
        ...
        Um, _, Vm = np.linalg.svd(
            (S - Sref) @ np.concatenate([Shat, Xi @ Poly]).T, full_matrices=False
        )
        Omega = Um @ Vm
        ...
        for snapshot in range(num_snapshots):
            ...
            Shat[:, snapshot] = least_squares(
                representation_learning_obj,
                Shat[:, snapshot],
                args=(snapshot, Vr, Vbar, Xi, S, sref, p),
                ftol=1e-9,
            ).x
        Poly = np.concatenate(polynomial_form(Shat, p=p), axis=0)
        ...
        energy = (
            np.linalg.norm(Vr @ Shat + (Vbar @ Xi @ Poly), "fro") ** 2
            / np.linalg.norm(S - Sref, "fro") ** 2
        )
...
def representation_learning_obj_jax(x, snapshot_index, Vr, Vbar, Xi, S, sref):
    ...
    return (
        ...
        - sref
        - (Vr @ x)
        - (Vbar @ Xi @ jnp.concatenate(polynomial_form(x, 2), axis=0))
    )
...
def alternating_minimization_jax(
    S, Sref, num_snapshots, max_iter, tol, gamma, r, q, p, initial_Shat=None
):
    """
    Performs alternating minimization to solve for Shat given the system parameters.
    ...
    Xi = (
        ...
        @ Proj_error
        @ Poly.T
        @ jnp.linalg.inv(Poly @ Poly.T + gamma * jnp.identity((p - 1) * r))
    )

    for niter in range(max_iter):
        # Step 1 - Orthogonal Procrustes (update basis vectors)
        Um, _, Vm = jnp.linalg.svd(
            (S - Sref) @ jnp.concatenate([Shat, Xi @ Poly]).T, full_matrices=False
        )
        Omega = Um @ Vm
        Vr, Vbar = Omega[:, :r], Omega[:, r : r + q]
        ...
        # Step 3 - Nonlinear regression (update reduced state representation)
        Shat = vmap(
            nonlinear_regression_step, in_axes=[0, None, None, None, None, None, None]
        )(jnp.arange(num_snapshots), Shat, Vr, Vbar, Xi, S, sref).T
        # Shat = jnp.array(Shat)
        # for snapshot in range(num_snapshots):
        ...
        # Evaluate convergence criterion
        energy = (
            jnp.linalg.norm(Vr @ Shat + (Vbar @ Xi @ Poly), "fro") ** 2
            / jnp.linalg.norm(S - Sref, "fro") ** 2
        )
        diff = abs(energy - nrg_old)
        print(f"\titeration: {niter+1:d}\tsnapshot energy: {energy:e}\t diff: {diff:e}")
        if diff < tol:
...
for q_trunc_index in range(len(q_trunc_lst)):
    ...
    relative_error_testing_window_lst_vary_q.append(relative_error_testing_window)
    relative_error_training_window_lst_vary_q.append(relative_error_training_window)


# save the results as pickle
with open("OpInfPoly_Results_Err_Analysis_vary_q.pkl", "wb") as f:
    pickle.dump(
        {
