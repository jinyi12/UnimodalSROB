...
function [D,l,c,s,mr,m, drp] = getDataMatrix(X,Vr,U,ind,modelform, p)
K = length(ind);
r = size(Vr,2);

% reduce X with removed points from X based on ind due to finite difference scheme, lost points at the edges of the data
Xhat = Vr'*X(:,ind);
m = size(U,2);
