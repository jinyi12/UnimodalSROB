...
def gen_tangent_samples(N_samples: int, beta: cp.Variable, X: NDArray, seed: int = 42):
    ...
    X: reshaped array of diffusion map basis, where X = np.reshape(Deltas[:num_models, :, :], (num_models, -1))
...
def gen_stiefel_samples(N_samples: int, rob: NDArray, tau: float = 1e-4, verbose: bool = True):
    ...
    # the global diffusion map basis as reference base point
    U0 = rob[-1]
    
    # number of models excluding the global diffusion map basis
    num_models = len(rob) - 1
    ...
    tangential_samples = gen_tangent_samples(N_samples, beta, X) 
